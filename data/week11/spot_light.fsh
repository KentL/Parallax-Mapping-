...
void main()
{
	...
	if( distance < 0 || distance > LightRange )
	{
	...
	else
	{
		...
		float spot = pow(max(dot(-lightDir,LightDir), 0.0), LightSpot);

		// Attenuate the light
		float att = min(1.0, spot / dot(LightAttenuation, vec3(1.0, distance, distance*distance)));
		light *= att;
